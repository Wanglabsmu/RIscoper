...
def get_entities(seq, suffix=False):
    ...
    # for nested list
    if any(isinstance(s, list) for s in seq):#Python中的isinstance()函数，isinstance()是Python中的一个内建函数。是用来判断一个对象的变量类型。
...
def f1_score(y_true: object, y_pred: object, average: object = 'micro', suffix: object = False) -> object:
    ...
    #true_entities = list(true_entities)
    #true_entities = true_entities.sort()
    pred_entities = set(get_entities(y_pred, suffix))
    #pred_entities = list(pred_entities)
    #pred_entities = pred_entities.sort()
